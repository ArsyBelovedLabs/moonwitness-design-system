...
export const CommandCenterBenchmark: Story = { render: () => page('command', <>
  ...
  <div className="mw-page-stage__body">
    <CommandDeck
      header={<ArchiveGate placeholder="Jump to evidence, source, map, review…" trailing={<span style={{fontSize:9,color:'var(--mw-accent-secondary)'}}>⌘ K</span>}/>} 
      ...
      spine={<EvidenceSpine steps={[{label:'CLAIM',detail:'captured',state:'complete'},{label:'SOURCE',detail:'linked',state:'complete'},{label:'CORROBORATE',detail:'cross-check',state:'complete'},{label:'REVIEW',detail:'active',state:'active'},{label:'ARCHIVE',detail:'bounded'}]}/>} 
      strip={<div style={{display:'grid',gap:10}}><TemporalOrrery activeIndex={4} points={['T-4','T-3','T-2','T-1','T0','T+1','T+2'].map(label=>({label}))}/><CausalityGuardrail/></div>}
    />
  </div>
</>) }
